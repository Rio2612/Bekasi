...
export default function Produk() {
  ...
  return (
    <section id="produk" className="py-[5.5rem] px-[5vw] bg-white">
      ...
      {/* PRODUK CARDS — semua cluster dirender di DOM (SSR), hanya cluster aktif yang ditampilkan secara visual.
          Ini memastikan seluruh 24 halaman produk punya link nyata di HTML awal, bukan cuma tab yang sedang aktif,
          supaya Googlebot bisa menemukan & merayapinya tanpa perlu mensimulasikan klik. */}
      {CLUSTERS.map(cluster => (
        <div
          key={cluster.id}
          className={`grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-6 ${cluster.id === activeId ? "grid" : "hidden"}`}
        >
          {cluster.produk.map(p => (
